...
function onCreate()
	...
	restart = false
	exit = false
	precacheSound('LullabyPause') -- pre cache pause song
	...
	addLuaSprite('bg')
	makeLuaSprite('left', 'pause/left', -1000, 0) -- left side
	setProperty('left.alpha', 1)
	setObjectCamera('left', 'other')
	setProperty('left.visible', false)
	addLuaSprite('left')
	makeLuaSprite('right', 'pause/right', 1200,0) -- right side
...
function onUpdate()
	if tinocoPaused == true then -- if paused
		...
		doTweenX('left', 'left', 0, 0.2, 'linear') -- left img slide anim
		doTweenX('right', 'right', 596, 0.2, 'linear') -- right img slide anim
...
function ILOVEBUTTONS()
	if keyJustPressed('pause') and die == 0 then -- if pressed pause button
		if tinocoPaused == false then -- if not paused
			...
			playSound('LullabyPause', 0, 'pausesong') -- play pause song
...
function onTimerCompleted(tag, loops, loopsLeft)
	if tag == 'loopmusic' then
		playSound('LullabyPause', 0.7, 'pausesong') -- loop song
	end
end